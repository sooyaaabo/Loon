    ...
    .map((it) => {
        ...
        if (fixedFestivalMatch) {
            ...
            if (defaultFestivalMap.hasOwnProperty(festival)) {
                return { name: festival, date: defaultFestivalMap[festival], diff: getDiffDays(defaultFestivalMap[festival]) }
            }
        } else {
...
const notify = async () => {
    ...
    let content = weekendDays === 0 ? `${timeFrame}好, 周末快乐\n ${(await getOneWord()) || `记得多陪陪家人哦~`}` : `距离周末还有${weekendDays}天, ${MOYU_COPY_WRITE[getWeekDay()]}`
    // 节日提醒
