...
if (reg1.test($request.url)) {
  ...
  $.setdata(null, "TestFlight_Header");
  const TF_header = {
  ...
  $.setdata(JSON.stringify(TF_header), "TestFlight_Header");

  if ($.getdata("TestFlight_Header") !== null) {
    if (sendNotify) {
...
if (reg2.test($request.url)) {
  let appId = $.getdata("TestFlight_APP_ID") || "";
  let arr = appId.split(",");
  ...
  $.setdata(JSON.stringify(appId), "TestFlight_APP_ID");
