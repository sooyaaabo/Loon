...
let TF_APP_ID = isNode ? process.env["TestFlight_APP_ID"] : $.getdata("TestFlight_APP_ID");
let TF_header = isNode ? process.env["TestFlight_Header"] : $.getdata("TestFlight_Header");
...
function updateData(ids, appId) {
  if (isNode) {
    ...
    const ids = $.getdata("TestFlight_APP_ID").split(',').filter(ids => ids.trim() !== appId);
    console.log(`TF_APP_ID即将更新为：${ids}`);
    $.setdata(ids.toString(), "TestFlight_APP_ID");
  }
}
